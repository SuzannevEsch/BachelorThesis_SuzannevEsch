...
output_excel_file = output_dir / "H2150_NI2_TWSO_WP_IRRTOT_PRTOT.xlsx"
...
def run_simulation(weather_file):
    ...
    agromanagement = YAMLAgroManagementReader(data_dir / "Agromanagement_2136_2165_no_irrigation2.agro")
    ...
    for year in range(2136, 2166):
        if f"{year}-03-14" in df_results.index.strftime('%Y-%m-%d'):
            for i in range(1, len(df_results)):
                start_date = pd.Timestamp(f"{year}-03-15")
                end_date = pd.Timestamp(f"{year}-08-15")
                if start_date <= df_results.index[i] <= end_date:
                    df_results.iloc[i, df_results.columns.get_loc('RAINT')] -= df_results.loc[f"{year}-03-14", 'RAINT']
                    df_results.iloc[i, df_results.columns.get_loc('TOTIRR')] -= df_results.loc[f"{year}-03-14", 'TOTIRR']

    # Filter data for the date 10-20 for every year
    filtered_data = df_results[df_results.index.strftime('%m-%d') == '08-15']
